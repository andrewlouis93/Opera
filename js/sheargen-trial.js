...
if (sessvars.damperType == "hyster"){
		alpha = 0.2*index3; //for hysteretic use 0.2, 0.4 and 0.6
	}
else{
...
var mu =10*index4;
var xi = 0.1*index4;
var muf = Rd(index1, index2, index3, index4)/Vf;
var mus = Rv(index1, index2, index3, index4)/(Vf^2/Rd(index1, index2, index3, index4)+(Rv(index1, index2, index3, index4)-Vf)/mu);
if (mus <= 1){
    mus = 1;
}
if (alpha == 1){
...
for (i=0; i<=n;i+=1){
   //stiffnesses
    k_frame[i] = Math.pow((2*Math.Math.PI/Tf),2)*sum_mphi_f[i]/dphi_f[i];
    ...
    if(i==1){
        ...
        y_frame[i]=Vf*Sd[index1]*Math.pow((2*Math.PI/Tf),2)*M1_f;
        y_frame_dummy[i] = Vf*Sd[index1]*Math.pow((2*Math.PI/Tf),2)*M1_f_dummy;
	}
    else{
...
for (i=0; i<=n;i+=1){
    K_frame[index1][index2][index3][index4][i] = k_frame[i];
    K_damper[index1][index2][index3][index4][i] = k_damper[i];            
...
if (rayleigh == 1){
		...
		xi1 = 0.05;
		high_mode = uint8(n*2/3)+1; //WHAT IS 'n'
		xim = 0.05;
		//df_count = 1;
		//df(1) = 1;
	}
    ...
    beta = 2*xi*sum_mphisq_f*(2*pi/Tf)/sum_dphisq_f_kd;
    console.log('beta for VE dampers = '+beta);
    for (var i=1; i<=n; i++){
        if (alpha != 1){
